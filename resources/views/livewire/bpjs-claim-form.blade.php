<div class="max-w-4xl mx-auto p-8 bg-gray-700 text-gray-100 rounded-xl shadow-lg">
    ...
    <form wire:submit.prevent="submit" wire:loading.attr="disabled" wire:target="submit" class="space-y-8">
        ...
        <div class="grid grid-cols-4 gap-6">
            ...
            <div class="col-span-1">
                <flux:input 
                    ...
                    label="Nomor RM" 
                    type="text" 
                    wire:model.lazy="no_rm"
                    icon:trailing="{{ $rmIcon }}" 
            ...
            <div class="col-span-2">
                <flux:input 
                    ...
                    readonly
                    class="cursor-not-allowed mt-1.5" 
                    label="Nama Pasien" 
            ...
            <div class="col-span-1">
                <flux:input 
                    ...
                    variant="filled" 
                    icon="credit-card" 
                    wire:model.debounce.500ms="no_kartu_bpjs"
                    readonly 
                    class="cursor-not-allowed mt-1.5" 
        ...
        <div class="grid grid-cols-4 gap-6">
            ...
            <div class="col-span-2">
                <flux:input 
                    type="text" 
                    icon="document-text" 
                    wire:model.debounce.500ms="no_sep" 
            ...
            <div>
                <flux:select 
                    wire:model="jenis_rawatan" 
                    label="Jenis Rawatan" 
                    placeholder="Pilih Jenis Rawatan"
                    badge="Wajib diisi"
                >
            ...
            <div>
                <flux:input 
                    type="date" 
                    wire:model="tanggal_rawatan" 
                    placeholder="Tanggal Rawatan"
                    label="Tanggal Rawatan" 
                    badge="Wajib diisi" 
                />
            </div>
        </div>
        ...
        <div>
            <flux:input 
                type="file" 
                label="Upload Scanned Documents" 
                wire:model="scanned_docs" 
                multiple
                accept=".pdf,.jpg,.png" 
                placeholder="Upload Scanned Documents" 
            />
        </div>
        @endif
        ...
        <div class="mt-4 space-y-2">
            ...
            <div class="flex items-center gap-4 p-4 bg-gray-700 border border-gray-600 rounded-lg">
                ...
                <div class="w-64 h-80 overflow-hidden border border-gray-500 rounded bg-white flex-shrink-0 shadow-sm relative">
                    <div class="w-full h-full" style="transform: rotate({{ ($rotations[$index] ?? 0) }}deg); transform-origin: center center; transition: transform 0.3s ease;">
                        <iframe 
                ...
                <div class="flex gap-2">
                    ...
                    <flux:button 
                        ...
                        size="xs" 
                        variant="ghost" 
                        wire:click.prevent="rotateFile({{ $index }})"
                        title="Putar PDF"
                    >
                        {{ $rotations[$index] ?? 0 }}°
                    </flux:button>
                    ...
                    <flux:button 
                        ...
                        size="xs" 
                        variant="ghost"
                        wire:click.prevent="previewFile({{ $index }})" 
                        title="Preview"
                    />
        ...
        @if($showPreviewModal)
        <div 
            class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-70 p-4 overflow-auto"
        ...
        >
            <div class="relative bg-gray-800 rounded-lg shadow-xl w-full max-w-6xl h-[90vh] flex flex-col">
                ...
                <div class="flex-shrink-0 flex justify-between items-center px-6 py-4 border-b border-gray-600">
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <flux:button 
                            icon="arrow-uturn-right" 
                            variant="ghost" 
                            wire:click="rotateFile({{ $currentPreviewIndex }})"
                            title="Putar PDF"
                        >
                            Putar
                        </flux:button>
                ...
                <div class="flex-1 min-h-0 relative">
                    ...
                    <div class="w-full h-full" style="transform: rotate({{ ($rotations[$currentPreviewIndex] ?? 0) }}deg); transform-origin: center center; transition: transform 0.3s ease;">
                        <iframe 
    ...
    <!-- Loading Overlays -->
    <div 
        wire:loading wire:target="scanned_docs"
    ...
    >
        <div class="flex flex-col items-center gap-4 text-center animate-fade-in">
            <svg class="h-12 w-12 animate-spin text-amber-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
    ...
    <div 
        wire:loading wire:target="new_docs"
        class="fixed inset-0 z-50 bg-black/60 flex items-center justify-center backdrop-blur-sm"
    >
        <div class="flex flex-col items-center gap-4 text-center">
            <svg class="h-10 w-10 animate-spin text-amber-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4" />
                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z" />
            </svg>
    ...
    >
        <div class="flex flex-col items-center gap-4 text-center">
            <svg class="h-12 w-12 animate-spin text-emerald-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4" />
                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z" />
            </svg>
